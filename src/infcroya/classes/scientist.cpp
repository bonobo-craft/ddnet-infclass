...
void CScientist::OnWeaponFire(vec2 Direction, vec2 ProjStartPos, int Weapon, CCharacter* pChr)
{
	...
	case WEAPON_HAMMER: {
		...
		while (p)
		{
			...
			if (p->GetOwner() == ClientID)
			{
				...
				if (d < 2.0f * g_Config.m_InfScientistMineRadius)
				{
					if (pIntersectMine)
						FreeSpace = false;
					else
			...
			else if (d < 2.0f * g_Config.m_InfScientistMineRadius)
				FreeSpace = false;
		...
		if (FreeSpace)
		{
			...
			else if (NbMine >= g_Config.m_InfScientistMineLimit && pOlderMine)
	...
	case WEAPON_GRENADE: {
		...
		if (length(PortalShift) > 2000.0f)
			PortalShift = PortalDir * 2000.0f;
		vec2 PortalPos;
