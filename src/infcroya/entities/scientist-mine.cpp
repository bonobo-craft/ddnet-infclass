...
void CScientistMine::Explode(int DetonatedBy)
{
	...
	if(OwnerChar)
	{
		...
		if(Dist < OwnerChar->GetProximityRadius()+g_Config.m_InfScientistMineRadius)
			OwnerChar->TakeDamage(vec2(0.0f, 0.0f), m_Pos, 4, DetonatedBy, WEAPON_HAMMER);
		else if(Dist < OwnerChar->GetProximityRadius()+2*g_Config.m_InfScientistMineRadius)
		{
			float Alpha = (Dist - g_Config.m_InfScientistMineRadius-OwnerChar->GetProximityRadius())/g_Config.m_InfScientistMineRadius;
...
{
	float Radius = g_Config.m_InfScientistMineRadius;
...
void CScientistMine::Tick()
{
	...
	for(CCharacter *p = (CCharacter*) GameWorld()->FindFirst(CGameWorld::ENTTYPE_CHARACTER); p; p = (CCharacter *)p->TypeNext())
	{
		...
		if(Len < p->GetProximityRadius()+g_Config.m_InfScientistMineRadius)
		{
			if(p->IsHuman()) {
				p->GetCroyaPlayer()->SetInsideSafeZone(true);
				continue;	
			}
			MustExplode = true;
